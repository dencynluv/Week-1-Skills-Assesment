...
def long_words(word_list):
    ...
    for word in word_list:
        if len(word) > 4:
            longer_then_four.append(word)
    return longer_then_four
...
def smallest_int(number_list):
    ...
    """
# Needs more work. Returns the first problem but not the second.
...
def largest_int(number_list):
    ...
    # else:
    #     return None
    # Can not figure out what to compare num to.
...
def word_lengths(word_list):
    ...
    for word in word_list:
        length = len(word)
        word_length_totals.append(length)

    return word_length_totals
...
def average(number_list):
    ...
    for num in number_list:
        add_nums = add_nums + num
        total = add_nums / float(len(num_list))

    return total

    # shorter way
    # num_list = number_list
    # total = sum(num_list) / float(len(num_list))
    # return total
...
def reverse_list_in_place(my_list):
    ...
    """
    my_list[:] = [3, 2, 1]
    # Replaces the elements in the list
    # this is the only way I could come up with that would work.
    # However, I don't think it would work for the strings in the 2nd test.
    # Also I think the function above is interfering with this one.
    # because it passed the test for both.
...
def duplicates(my_list):
    ...
    # This only returns the list back with no duplicates.
    # because I am passing it into a set.
    # It doesn't pull out the duplicates.
